...
namespace Seq.Input.MsSql
{
    public class Executor
    {
        ...
        {
            var currentState = Interlocked.CompareExchange(ref SqlConfig.SqlLockState, SqlConfig.Locked, SqlConfig.Available);

            //Allow for long running queries to block further queries until timeout
            if (currentState == SqlConfig.Available)
                try
                {
                ...
                finally
                {
                    ...
                    currentState = Interlocked.CompareExchange(ref SqlConfig.SqlLockState, SqlConfig.Available, SqlConfig.Locked);
                }
        }
